...
const Products_List = () => {
  ...
  const data = products_data[`${products}`];
    return (
<div className="font-[sans-serif]">
      <div className="p-4 mx-auto lg:max-w-6xl max-w-xl md:max-w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {
            data.map((data) => (
              <div key = {data.id} className="bg-gray-100 rounded-2xl p-6 cursor-pointer hover:-translate-y-2 transition-all relative">
                ...
                <div className="max-lg:w-11/12 w-4/5 h-[220px] overflow-hidden mx-auto aspect-w-16 aspect-h-8">
                  <img src={data.img} alt="Product 1" className="h-full w-full object-contain" />
                </div>
                <div className="text-center mt-4">
                  <h3 className="text-lg font-bold text-gray-800">{data.name}</h3>
                  <h4 className="text-xl text-gray-700 font-bold mt-4">{data.price1} <span className="text-gray-400 ml-2 font-medium">{data.price2}</span></h4>
                  <Link to={`/details2?product=${products}&id=${data.id}`}>
                    <button type="button" className="w-full mt-6 px-4 py-3 bg-[#333] hover:bg-[#222] text-white rounded-full">Shop Now</button>
                  </Link>
                </div>
              </div>
            ))
